...
Cat Breed Assistant is a small educational application that answers user questions about cat breeds. It separates the user interface, backend API, local breed data retrieval and answer providers while keeping the architecture intentionally simple.

The app supports a local mock mode for demos without API keys and optional LLM modes through OpenAI and Gemini.

## Architecture

```text
Streamlit frontend → FastAPI backend → breed retriever → Mock/OpenAI/Gemini provider
```
...
- OpenAI and Gemini modes that use retrieved breed context.
