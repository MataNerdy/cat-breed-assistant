...
Cat Breed Assistant is a small educational application that answers user questions about cat breeds. It separates the user interface, backend API, local breed data retrieval, optional vector RAG retrieval and answer providers while keeping the architecture understandable.
...
```text
Streamlit frontend → FastAPI backend → breed retriever / RAG retriever → Mock/OpenAI/Gemini/Mistral provider
```

The frontend sends questions to the backend over HTTP. The backend retrieves relevant breed facts from a local JSON file, can optionally retrieve semantic chunks from a local ChromaDB index, and routes the request to the selected answer provider.
...
- Optional full RAG pipeline from Hugging Face dataset preprocessing to ChromaDB retrieval.
...
- How to create a simple vector retrieval pipeline without LangChain or LlamaIndex.
- How to keep an app usable when the RAG index is not built yet.
...
- Add tests for the RAG preprocessing and retrieval contract.
